function init() {
  ...
  const updateStatus = () =>{
    const { calcWidth, calcHeight, maxWidth, cellD, ratio } = imgData
    indicator.innerHTML = `calcWidth: ${calcWidth} calcHeight: ${calcHeight} maxWidth: ${maxWidth} cellD: ${cellD} ratio: ${ratio}`
  }

  radioInputs.forEach(radioInput =>{
    radioInput.addEventListener('change', e => {
      imgData.size = e.target.value
      const { maxWidth, size } = imgData
      imgData.ratio = size_stat[size].maxWidth / maxWidth
